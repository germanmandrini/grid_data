...
#Keep the rasters in the P drive for now
                "~/Project.Grid/Grid/CDL/2008_30m_cdls"

CDL08 <- raster('~/Project.Grid/Grid/CDL/2008_30m_cdls/2008_30m_cdls.img')
setwd('~/Project.Grid/Grid/CDL/2008_30m_cdls')
CDL08 <- raster('2008_30m_cdls.img')
list.dirs()
...
for(tile_n in unique(grid5000_tiles.sf$id_tile)){
  ...
  for(cell_n in unique(ids_5000_seq$id_5000)){
    #cell_n=150000
    print(cell_n)
    ...
    for(year.n in 1:10){
      ...
      tbl <- tbl[id.crop %in% cdl.info.dt$id.crop[14]][,id.crop := as.numeric(id.crop)]
      tbl[,year := year.real]
      tbl[,source := 'crop.CDL']
      tbl[,unit := 'count30x30']
