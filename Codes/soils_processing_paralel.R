...
soil_aggr <- function(cell_n){
  ...
  library(sf)
    ...
    # cell_n = 14
    one_cell.sf <- grid5000_WGS84.sf[grid5000_WGS84.sf$id_5000 == cell_n,]
    ...
    cell_weather.dt <- data.table(id_tile = one_cell.sf$id_tile,
                                    id_5000 = cell_n,
                                    value = ext.mt2)
  return(cell_weather.dt)
}

keep <- c('keep', 'soil_aggr', 'grid5000_WGS84.sf', 'soil_variable_tmp_crop.r')

#rm(list = ls()[!ls() %in% keep])

clusterExport(cl, varlist = keep, envir=environment())

cell_seq <- grid5000_WGS84.sf$id_5000          
