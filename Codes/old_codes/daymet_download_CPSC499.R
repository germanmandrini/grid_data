...
library(nngeo)
# library('fasterize')
source('./Project.Grid/Grid/Codes/functions.grid_CPSC499.R')
...
source('C:/Users/germanm2/Box Sync/My_Documents/Project.Grid/Grid/Codes/functions.grid_CPSC499.R')
# folder_name = 'T:/germanm2/Grid/daymet/daily_data' #Dell

#list.files(folder_name)

grid5000.sf <- readRDS('./Project.Grid/Grid/rds.files/grid5000_tiles.sf.rds')
...
crs_5000 <- st_crs(grid5000.sf)
champaign.county <- projectRaster(champaign.county, crs = crs_5000$proj4string)
grid5000.sf <- st_crop(grid5000.sf, champaign.county)
...
for(row_n in 1:nrow(all_combinations)){
  ...
  #Open the raw files
  file.brk <- open_project_crop(files.year.full)
  ...
  if(!only_july){
    ...
    file_season_brk <- if(var_n == 'prcp'){
      ...
      #   tm_layout(legend.text.size = 0.7,
      #             main.title = paste('Time aggregation'),
      #             main.title.position = "center",
      #             main.title.size = 1.5)
      # 
