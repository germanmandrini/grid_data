...
if(FALSE){
  ...
  grid5000_wgs.spdf <- as(grid5000_wgs.sf, 'Spatial') #for the server
  saveRDS(grid5000_wgs.spdf, './Project.Grid/Grid/rds.files/grid5000_wgs.spdf.rds')
  }
...
for(row_n in 1:nrow(all_combinations)){
  ...
  #Open the raw files
  file_monthly_brk <-  suppressWarnings(raster::brick(files.year.full))
